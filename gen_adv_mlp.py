...
# --- Evaluate & save (MLP) ---
def evaluate_and_save_mlp(dataset_name, model, loader, attack_fn, attack_name, eps_params):
    ...
    # output to console + results/mlp
    print(msg)
    path = os.path.join('results/mlp', f"{dataset_name}_{attack_name}_results.txt")
...
if __name__ == '__main__':
    ...
    mnist_model.load_state_dict(torch.load('mnist_mlp.pt', map_location=device))
    cifar_model = CIFAR10_MLP().to(device)
    cifar_model.load_state_dict(torch.load('cifar10_mlp.pt', map_location=device))
